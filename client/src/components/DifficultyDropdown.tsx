...
const DifficultyDropdown = () => {
  ...
  return (
    <div>
      {!roomId ? (
        <div className="flex items-baseline text-base max-[340px]:text-xs">
          <p className="mr-2 font-semibold text-black">Difficulty:</p>
          <select
            defaultValue={difficulty}
            className="w-24 cursor-pointer border-b border-b-gray-400 bg-inherit text-gray-500 max-[340px]:w-16"
            ref={selectRef}
            onChange={handleDifficulty}
          >
            {diffOptions.map((opt) => (
              <option key={opt.id} value={opt.option} className="font-semibold">
